    ...
    <v-row wrap>
      <v-col cols="12" md="4" sm="4" class="green lighten-3">
        ...
        <v-card class="grey lighten-2 ml-6 mr-6 mt-8 mb-8 pb-4 pt-4 rounded-corner" v-for="note in listNotes" :key="note._id">
          ...
          <v-btn fab depressed small light text right href="" @click="deleteNotes(note._id)"><v-icon small>fa-trash</v-icon></v-btn>
          
          <v-dialog max-width="600px" v-model="dialog">
          <template v-slot:activator="{on}">
          <v-btn fab depressed small light text right v-on="on" @click="editnotes(note._id)"><v-icon small>fa-edit</v-icon></v-btn>
          </template>
